...
int PacketWindow::ReceivePacket(char* data, unsigned int seq, int length) {
	...
	if ((int)(lastSeq - (seq + length)) >= 0) return 0;
	...
	if (currentUB == nullptr && prevUB == nullptr) {
		...
		if ((int)(seq + length - (lastSeq + emptyLength + LinkSize * windowLinkCount)) > 0) {
			LinkedDataNode* newNode = new LinkedDataNode(window_tail);
			window_tail = newNode;
...
void TCPAssignment::syscall_listen(UUID syscallUUID, int pid, int fd, int capacity) {
  ...
  this->backlogMap[pid].listening_socket = &(this->socketMap[pid][fd]);
  this->backlogMap[pid].accept_hanging = false;
  this->returnSystemCall(syscallUUID, 0);
};
...
void TCPAssignment::syscall_accept(UUID syscallUUID, int pid, int fd, sockaddr* addrPtr, socklen_t* addrLenPtr) {
	if ( this->backlogMap[pid].q.empty() ) {
		...
		//now, accept call is hanging
		this->socketMap[pid][fd].syscallUUID = syscallUUID;
		this->socketMap[pid][fd].timerUUID = this->addTimer(tp, 100000000000000U);
		this->socketMap[pid][fd].addrPtr = addrPtr;
		this->socketMap[pid][fd].addrLenPtr = addrLenPtr;
		this->backlogMap[pid].accept_hanging = true;
		return;
	}

	//there is already handshaken socket in backlog.
...
void TCPAssignment::syscall_connect(UUID syscallUUID, int pid, int fd, sockaddr* addrPtr, socklen_t addrLen) {
  ...
  uint16_t window = htons(51123), checksum = 0, urgent = 0, newChecksum;
...
//   uint32_t ackResponse = htonl(this->socketMap[pid][connectedfd].rcvWindow->lastSeq);
// 	uint8_t headLen = 5 << 4;
// 	uint8_t flag = FIN;
// 	uint16_t window = 0;
// 	checksum = 0;
...
// 	response.writeData(TCP_START+4, &ackResponse, 4);
// 	response.writeData(TCP_START+8, &seqBuf, 4);
...
void TCPAssignment::packetArrived(std::string fromModule, Packet &&packet) {
  ...
  //std::cout << "packet flag: " << (int)flags << std::endl;
  ...
  switch(flags) {
    ...
    { 
		...
		for (std::map<int, std::map<int, socket>>::iterator itPid = this->socketMap.begin(); itPid != this->socketMap.end(); itPid++) {
			...
			if (
				(itFd->second.state == TCP_LISTEN) && //prev: || itFd->second.state == TCP_SYN_SENT) &&
				(
		...
		if (
			...
			printf("couldn't find listening socket\n");
			return;
		}

		this->backlogMap[pid].current++;

		printf("connection request arrived, fd created\n");
		...
		this->socketMap[pid][newfd].timerUUID = this->socketMap[pid][listeningfd].timerUUID;
		this->socketMap[pid][newfd].syscallUUID = this->socketMap[pid][listeningfd].syscallUUID;
		this->socketMap[pid][newfd].addrPtr = this->socketMap[pid][listeningfd].addrPtr;
		this->socketMap[pid][newfd].addrLenPtr = this->socketMap[pid][listeningfd].addrLenPtr;

 	 	int randSeq = rand();
		uint32_t ackResponse = htonl( ntohl(*(uint32_t *)seqBuf) + 1);
		newHeadLen = 5 << 4;
		newFlags = SYN | ACK;
		...
		p.writeData(TCP_START+8, &ackResponse, 4);
		p.writeData(TCP_START+12, &headLen, 1);
		...
		std::cout << "backlog len: " << this->backlogMap[pid].q.size() << std::endl;
		this->sendPacket("IPv4", std::move(p));
		printf("seq-ack packet sent\n");
      	return;
    }
    case (SYN | ACK):
    {
		...
		for (std::map<int, std::map<int, socket>>::iterator itPid = this->socketMap.begin(); itPid != this->socketMap.end(); itPid++) {
			for (std::map<int, socket>::iterator itFd = itPid->second.begin(); itFd != itPid->second.end(); itFd++) {
				//test
				std::cout << itFd->second.state << std::endl;
			if (
				(itFd->second.state == TCP_SYN_SENT) && //prev:  || itFd->second.state == TCP_LISTEN) &&
      ...
      for (std::map<int, std::map<int, socket>>::iterator itPid = this->socketMap.begin(); itPid != this->socketMap.end(); itPid++) {
        for (std::map<int, socket>::iterator itFd = itPid->second.begin(); itFd != itPid->second.end(); itFd++) {
          if (
			...
			printf("serverside handshake done\n");
			this->socketMap[pid][fd].state = TCP_ESTABLISHED;
			this->socketMap[pid][fd].rcvWindow = nullptr;
			this->socketMap[pid][fd].localseq = ntohl( *(uint32_t *)ackBuf );
			this->socketMap[pid][fd].remoteseq = ntohl( *(uint32_t *)seqBuf );

			if(this->backlogMap[pid].accept_hanging){ //accept call이 대기 중인 경우.
				memcpy(this->socketMap[pid][fd].addrPtr, &this->socketMap[pid][fd].remoteAddr, sizeof(sockaddr_in));
				*(this->socketMap[pid][fd].addrLenPtr) = sizeof(sockaddr_in);

				this->cancelTimer(this->socketMap[pid][fd].timerUUID);
				this->returnSystemCall(socketMap[pid][fd].syscallUUID, fd);
				this->backlogMap[pid].accept_hanging = false;
				printf("accept call late return\n");
			}else{
            ...
            itFd->second.state == TCP_ESTABLISHED &&
            itFd->second.remoteAddr.sin_addr.s_addr == ipSrc &&
            itFd->second.remoteAddr.sin_port == portSrc
          ) {
            pid = itPid->first;
            int connectedfd = itFd->first;
			...
			if(htons(packetLength) == 40){ //write한 것의 ack 반환이 도착한 경우.
			  ...
			  if(this->socketMap[pid][connectedfd].rcvWindow == nullptr){
				  printf("window not exist\n");
				  this->socketMap[pid][connectedfd].rcvWindow = 
				  	new PacketWindow(&(this->socketMap[pid][connectedfd].rcvBuffer), ntohl(*((unsigned int*)seqBuf)));
			  }
              ...
              this->socketMap[pid][connectedfd].rcvWindow->ReceivePacket(rcvBuf, (unsigned int)(ntohl(*((int*)seqBuf))), htons(packetLength) - 40);
			  this->socketMap[pid][connectedfd].remoteseq = (unsigned int)(ntohl(*((int*)seqBuf))) + htons(packetLength) - 40;

              //ack 전송 필요.
			  	uint32_t ackResponse = htonl(this->socketMap[pid][connectedfd].rcvWindow->lastSeq);
				//std::cout << "last seq: " << this->socketMap[pid][connectedfd].rcvWindow->lastSeq << std::endl;
				newHeadLen = 5 << 4;
				newFlags = ACK;
				...
				if(this->socketMap[pid][connectedfd].readHanging){
					this->socketMap[pid][connectedfd].readHanging = false;
					int returnVal = this->socketMap[pid][connectedfd].rcvBuffer.popData(this->socketMap[pid][connectedfd].read_dst, this->socketMap[pid][connectedfd].read_size);
					this->returnSystemCall(this->socketMap[pid][connectedfd].syscallUUID, returnVal);
				}
            	return;
            }
          }
	...
	//이쪽으로 넘어오지 않게 할 것.
	  ...
	  //과제 3 Accept용
		...
		uint32_t seqRes = *((uint32_t*)ackBuf);
		this->socketMap[pid][fd].rcvWindow = nullptr;
		this->socketMap[pid][fd].localseq = ntohl(ackRes);
		this->socketMap[pid][fd].remoteseq = ntohl(seqRes);
	  //과제 3번끝
		...
		for (std::map<int, std::map<int, socket>>::iterator itPid = this->socketMap.begin(); itPid != this->socketMap.end(); itPid++) {
			for (std::map<int, socket>::iterator itFd = itPid->second.begin(); itFd != itPid->second.end(); itFd++) {
				if (
					...
					std::cout << "backlog len: " << this->backlogMap[pid].q.size() << std::endl;
					//backlog에서 대기 중인 것이면, 미리 삭제해 준다.

					if(this->socketMap[pid][fd].state == TCP_SYN_RCVD){
						//해당 fd는 필요 없음.
						printf("tcp_syn_rcvd connection halt\n");
						this->socketMap[pid].erase(fd);
						this->removeFileDescriptor(pid, fd);
					}

					// for(int i = 0; i< this->backlogMap[pid].q.size(); i++){
					// 	if(this->backlogMap[pid].q.front() == fd){
					// 		printf("that socket found on backlog\n");
					// 		this->backlogMap[pid].q.pop();
					// 		this->socketMap[pid].erase(fd);
					// 		this->removeFileDescriptor(pid, fd);
					// 		this->backlogMap[pid].current--;
					...
					// 	this->backlogMap[pid].q.push(this->backlogMap[pid].q.front());
					// 	this->backlogMap[pid].q.pop();
					// }
